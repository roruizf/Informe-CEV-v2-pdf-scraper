...
SAMPLE_PDF_PRECAL_PATH = os.path.join(
    REPORT_EXAMPLES_FOLDER, SAMPLE_PDF_PRECAL_NAME)
...
RENAME_MAP_P5_P6 = {
    'codigo_evaluacion': 'Código Evaluación', 'content_note': 'Nota'}
...
def is_valid_cev_v2_pdf(pdf_doc: fitz.Document) -> bool:
    ...
    try:
        ...
        if valid:
            ...
            logging.warning("Validation failed: Keywords not found.")
        return valid
...
def display_dataframe_with_title(title: str, data: pd.DataFrame, transpose: bool = False, rename_map: Optional[Dict[str, str]] = None):
    ...
    placeholder_note_col = rename_map.get(
        'content_note', 'content_note') if rename_map else 'content_note'
    ...
    if is_placeholder and placeholder_note_col in data_to_display.columns:
        ...
        if pd.notna(note):
            st.info(note)
        if transpose:
            cols_to_drop = [placeholder_note_col]
            display_data = data_to_display.drop(
                columns=cols_to_drop, errors='ignore').T
...
def create_multisheet_excel(dataframes_list: List[pd.DataFrame], sheet_names: List[str], rename_maps: List[Optional[Dict[str, str]]]) -> BytesIO:
    ...
    with pd.ExcelWriter(excel_buffer, engine='openpyxl') as writer:
        if len(dataframes_list) != len(sheet_names) or len(dataframes_list) != len(rename_maps):
            logging.error(
                "Mismatch between dataframes, sheet names, or rename maps for Excel generation.")
        ...
        for i, df_original in enumerate(dataframes_list):
            if df_original is None or df_original.empty:
                logging.info(
                    f"Skipping empty dataframe for sheet '{sheet_names[i]}'")
            ...
            if rename_map:
                cols_to_rename = {
                    k: v for k, v in rename_map.items() if k in df_to_write.columns}
            ...
            try:
                ...
                logging.error(
                    f"Error writing sheet '{safe_sheet_name}': {e}", exc_info=True)
...
def main():
    ...
    if st.session_state.processing_done and st.session_state.extracted_data:
        ...
        if len(ALL_RENAME_MAPS) == len(st.session_state.extracted_data):
            ...
            st.warning(
                "No se pudo preparar el archivo Excel (error interno de mapeo).")

    # --- Define Tabs ---
    tab_titles = ["Subir Archivo PDF", "Página 1", "Página 2",
                  "Página 3", "Página 4", "Página 5", "Página 6", "Página 7"]
    tab_upload, tab_p1, tab_p2, tab_p3, tab_p4, tab_p5, tab_p6, tab_p7 = st.tabs(
        tab_titles)

    # --- Upload Tab Content ---
    with tab_upload:
        st.header("Cargar Archivo PDF")
        uploaded_file_widget = st.file_uploader("Seleccione un archivo PDF (informe_cev_v2)", accept_multiple_files=False,
                                                type="pdf", key="pdf_uploader", help="Suba un archivo PDF en formato 'Informe CEV v2'.")

        if uploaded_file_widget is not None:
            current_file_id = uploaded_file_widget.file_id
            if current_file_id != st.session_state.last_uploaded_file_id:
                st.info(
                    f"Nuevo archivo detectado: '{uploaded_file_widget.name}'. Procesando...")
                st.session_state.uploaded_file_bytes = uploaded_file_widget.getvalue()
                st.session_state.file_name = uploaded_file_widget.name
                ...
                try:
                    with fitz.open(stream=st.session_state.uploaded_file_bytes, filetype="pdf") as pdf_doc:
                        ...
                        if not is_valid_cev_v2_pdf(pdf_doc):
                            st.error(
                                f"Archivo '{st.session_state.file_name}' inválido o no soportado.")
                            reset_state()
                            processing_placeholder.empty()
                        else:
                            processing_placeholder.info(
                                f"Archivo válido. Procesando '{st.session_state.file_name}'...")
                            extracted_dfs, base_names = process_pdf(
                                pdf_doc, st.session_state.file_name)
                            st.session_state.extracted_data = extracted_dfs
                            st.session_state.processing_done = True
                            processing_placeholder.empty()
                            st.success(
                                f"Procesamiento de '{st.session_state.file_name}' completado.")
                            # Use st.rerun() to refresh the interface cleanly after processing
                            st.rerun()
                except Exception as e:
                    error_message = str(e).lower()
                    user_msg = f"Error inesperado procesando {st.session_state.file_name}: {e}"
                    if any(err in error_message for err in ["cannot open", "damaged", "format error", "no objects found"]):
                        user_msg = f"Error al leer PDF {st.session_state.file_name}. Podría estar dañado/protegido."
                    log_msg = f"Error processing file {st.session_state.file_name}: {e}"
                ...
                st.success(
                    f"Archivo '{st.session_state.file_name}' procesado correctamente.")
            elif st.session_state.file_name:
                st.warning(
                    f"Archivo '{st.session_state.file_name}' cargado, pero hubo error.")
        else:
            if st.session_state.last_uploaded_file_id is not None:
                reset_state()
            if not st.session_state.processing_done:
                st.info("Por favor, seleccione un archivo PDF.")
        # --- Example File Download ---
        st.markdown("---")
        st.subheader("Archivos de Ejemplo")
        st.markdown("""
    ...
    data_structure = {
        ...
        # P2
        1: ("Información Gral. / Demanda energética / Diseño de Arquitectura", 1, True, RENAME_MAP_P2),
        ...
        # P4
        3: ("Demanda, sobrecalentamiento y sobreenfriamiento mensual", 4, False, RENAME_MAP_P4),
        # P5 (Placeholder) - Transpose=True
        4: ("FLUJOS ENERGÉTICOS ACUMULADOS MENSUALES".title(), 5, True, RENAME_MAP_P5_P6),
        # P6 (Placeholder) - Transpose=True
        5: ("Temperaturas Interior y Exterior diarias", 6, False, RENAME_MAP_P5_P6),
        # P7
        ...
        with tab:
            if st.session_state.processing_done and st.session_state.extracted_data and len(st.session_state.extracted_data) == 8:
                if st.session_state.file_name:
                    st.caption(
                        f"Mostrando resultados para: {st.session_state.file_name}")
                structure_info = data_structure[i]
                if isinstance(structure_info, list):  # Handle combined Page 3
                    part1 = structure_info[0]
                    part2 = structure_info[1]
                    display_dataframe_with_title(
                        part1[0], st.session_state.extracted_data[part1[1]], transpose=part1[2], rename_map=part1[3])
                    st.markdown("---")
                    display_dataframe_with_title(
                        part2[0], st.session_state.extracted_data[part2[1]], transpose=part2[2], rename_map=part2[3])
                    ...
                    display_dataframe_with_title(
                        title, st.session_state.extracted_data[df_index], transpose=transpose_flag, rename_map=current_rename_map)
            else:
                st.info(
                    "Suba un archivo PDF válido en la pestaña 'Subir Archivo PDF' para ver los datos.")
